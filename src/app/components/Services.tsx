      ...
      <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
        {services.map((service, index) => (
          ...
          >
            ...
            <div className={`absolute inset-0 bg-linear-to-br ${service.bgGradient} opacity-0 group-hover:opacity-100 transition-opacity duration-300`}></div>
            
            {/* Content */}
            <div className="relative p-8">
              {/* Icon */}
              <div className={`inline-flex items-center justify-center w-16 h-16 rounded-xl bg-linear-to-br ${service.gradient} text-white mb-6 transform group-hover:scale-110 group-hover:rotate-3 transition-transform duration-300`}>
                {service.icon}
              </div>
              ...
              <div className="flex items-center text-sm font-semibold">
                <span className={`bg-linear-to-r ${service.gradient} bg-clip-text text-transparent group-hover:translate-x-2 transition-transform duration-300`}>
                  Saiba mais
                </span>
                <svg 
                  className={`w-5 h-5 ml-2 text-blue-600 group-hover:translate-x-2 transition-transform duration-300`} 
                  fill="none" 
                  stroke="currentColor" 
                  viewBox="0 0 24 24"
                >
            ...
            {/* Decorative Element */}
            <div className={`absolute -bottom-2 -right-2 w-24 h-24 bg-linear-to-br ${service.gradient} opacity-10 rounded-full blur-2xl group-hover:opacity-20 transition-opacity duration-300`}></div>
          </div>
        ))}
      </div>
    </Section>
